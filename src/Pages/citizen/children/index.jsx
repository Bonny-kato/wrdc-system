...
const Children = () =>{
    ...
    const {children} = useGlobalContext();
                   ...
                   <div className={'grid grid-cols-3 gap-x-10'}>
                       <div className={' col-span-2 grid gap-8 grid-cols-4'}>
                           {children.map((children, idx) => (
                               <CitizenCard citizen={children} setCurrentCitizen={setCurrentChildren} />
                           ))}
                       </div>
                       {currentChildren && <CitizenDetails currentCitizen={currentChildren}/>}
                   </div>
               </div>
            </div>
        </BaseLayout>
